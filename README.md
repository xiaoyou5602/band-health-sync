...
- **自托管健康同步**：把手环的步数、心率、睡眠直接从蓝牙同步、上传到你自己的服务器，
  再通过 MCP 给 AI 助手读取，不经过任何厂商云。
- **修复华为「科学睡眠」被静默关闭**：上游每次连接都会向华为设备下发「关闭 TruSleep」，
  导致手环睡眠丢失。本 fork 只在用户显式操作时才改动该设置。
...
下方保留上游项目原始 README，用于查看通用功能、设备支持和开发资料。
