...
public class SleepAnalysisTest {
    ...
    @Test
    public void testSessionsAreAttributedToWakeDate() {
        TimeZone timeZone = TimeZone.getTimeZone("Asia/Singapore");
        Calendar septemberFirst = day(timeZone, 2026, Calendar.SEPTEMBER, 1);
        Calendar septemberSecond = day(timeZone, 2026, Calendar.SEPTEMBER, 2);

        int sleepStart = timestamp(timeZone, 2026, Calendar.SEPTEMBER, 1, 23, 0);
        int sleepEnd = timestamp(timeZone, 2026, Calendar.SEPTEMBER, 2, 7, 0);
        List<ActivitySample> samples = Arrays.asList(
                sleep(sleepStart, ActivityKind.LIGHT_SLEEP),
                sleep(sleepEnd, ActivityKind.DEEP_SLEEP),
                activity(sleepEnd + 1, 100)
        );

        SleepAnalysis analysis = new SleepAnalysis();
        assertEquals(0, analysis.calculateSleepSessionsForWakeDate(samples, septemberFirst).size());
        assertEquals(1, analysis.calculateSleepSessionsForWakeDate(samples, septemberSecond).size());
        assertEquals(8 * 60 * 60,
                analysis.calculateSleepAmountsForWakeDate(samples, septemberSecond).getTotalSeconds());
    }

    private static Calendar day(TimeZone timeZone, int year, int month, int date) {
    ...
    private static int timestamp(TimeZone timeZone, int year, int month, int date, int hour, int minute) {
        Calendar calendar = new GregorianCalendar(timeZone);
        calendar.clear();
