...
public class SleepDailyFragment extends SleepFragment<SleepDailyFragment.MyChartsData> {
    ...
    protected MyChartsData refreshInBackground(ChartsHost chartsHost, DBHandler db, GBDevice device) {
        ...
        wakeDate.setTime(getEndDate());
        List<ActivitySample> samples = new ArrayList<>(
                DailyTotals.getSleepSamplesForWakeDate(db, wakeDate, device)
        );
        List<? extends SleepScoreSample> sleepScoreSamples = new ArrayList<>();
        if (supportsSleepScore()) {
            sleepScoreSamples = getSleepScoreSamples(db, device, wakeDate);
        }
        MySleepChartsData mySleepChartsData = refreshSleepAmounts(samples, sleepScoreSamples, wakeDate);
        ...
        if (!mySleepChartsData.sleepSessions.isEmpty()) {
            ...
            for (Iterator<ActivitySample> iterator = samples.iterator(); iterator.hasNext(); ) {
    ...
    private MySleepChartsData refreshSleepAmounts(
            List<? extends ActivitySample> samples,
            List<? extends SleepScoreSample> sleepScoreSamples,
            Calendar wakeDate
    ) {
        SleepAnalysis sleepAnalysis = new SleepAnalysis();
        List<SleepSession> sleepSessions = sleepAnalysis.calculateSleepSessionsForWakeDate(samples, wakeDate);
