...
public class DailyTotals implements Serializable {
    ...
    public DailyTotals() {
        this(0, 0, new long[]{0, 0, 0 ,0}, 0, 0, -1);
    }
    ...
    public DailyTotals(final long steps, final long distance, final long[] sleep, final long activeCalories, final long restingCalories) {
        this(steps, distance, sleep, activeCalories, restingCalories, -1);
    }

    public DailyTotals(final long steps, final long distance, final long[] sleep, final long activeCalories, final long restingCalories, final int heartRate) {
        this.steps = steps;
    ...
    public static DailyTotals getDailyTotalsForDevice(GBDevice device, Calendar day, DBHandler handler) {
        ...
        List<? extends ActivitySample> samplesOfDay = getSamplesOfDay(handler, day, 0, device);
        totalAmounts = analysis.calculateActivityAmounts(samplesOfDay);
        amountsSleep = new SleepAnalysis().calculateSleepAmountsForWakeDate(
                getSleepSamplesForWakeDate(handler, day, device),
                day
        );
        ...
        int latestHeartRate = calculateLatestHeartRate(
                samplesOfDay,
                heartRateUtils.getMinHeartRate(),
                heartRateUtils.getMaxHeartRate()
        );

        // Purposely not including awake sleep
        return new DailyTotals(totalSteps, totalDistance, sleep, totalActiveCalories, totalRestingCalories, latestHeartRate);
    }

    static int calculateLatestHeartRate(
    ...
    public static List<? extends ActivitySample> getSleepSamplesForWakeDate(
            DBHandler db,
            Calendar wakeDate,
            GBDevice device
    ) {
        Calendar start = (Calendar) wakeDate.clone();
        ...
        Calendar end = (Calendar) wakeDate.clone();
        ...
        int startTs = (int) (start.getTimeInMillis() / 1000L);
        int endTs = (int) (end.getTimeInMillis() / 1000L - 1L);
        return getSamples(db, device, startTs, endTs);
    }
