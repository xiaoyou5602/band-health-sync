...
abstract class PeriodicExporter {
    ...
    fun scheduleNextExecution(context: Context) {
        try {
            ...
            LOG.info(
                "Scheduling {} with a {}-minute interval from {} (initial delay: {}ms)",
    ...
    /**
     * Coalescing variant of [executeNow] for event-driven exports (toge's fork).
     *
     * Every export rewrites the whole file, and a single fetch can emit more than one
     * "new data" event. Re-arming a unique work request collapses a burst into one export,
     * [delaySeconds] after the last event.
     */
    fun executeDebounced(delaySeconds: Long) {
        val workManager = WorkManager.getInstance(GBApplication.getContext())
        val exportRequest = OneTimeWorkRequest.Builder(getWorkerClass())
