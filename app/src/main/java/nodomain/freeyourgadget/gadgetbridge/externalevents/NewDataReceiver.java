...
public class NewDataReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (deviceAddress == null || deviceAddress.isBlank()) {
            ...
            LOG.warn("ACTION_NEW_DATA received without device information, skipping sync");
            return;
        }

        final GBPrefs prefs = GBApplication.getPrefs();
        // The two destinations are independent: a user can feed Health Connect, their own server,
        // or both, and one being switched off must not silence the other.
        scheduleHealthConnectSync(context, prefs, deviceAddress);
        scheduleSelfHostedHealthSync(context, prefs, deviceAddress);
    }

    private void scheduleHealthConnectSync(Context context, GBPrefs prefs, String deviceAddress) {
